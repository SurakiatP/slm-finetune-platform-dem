...
| [012](./ADR-012-owner-mismatch-403-not-404.md) | Owner-mismatch on an existing resource returns `403`, not `404` — genuinely-missing resources stay `404`; supersedes the "404, not 403" rule inside ADR-009's Ownership section | Accepted |
| [013](./ADR-013-metrics-prometheus.md) | Prometheus metrics: scrape-time derivation (never in-worker counters), a fixed non-tenant label set, `GET /metrics` unauthenticated but not edge-proxied, Grafana deferred, GPU exporter shipped expected-down, and per-queue `/ready` worker probes | Accepted |
