...
| MT.6 | **Manual Swagger pass (parks)** — walk all 5 new runbooks + verify content matches reality | ⏳ | Forward port 8000 (`-L 8000:localhost:8000`); recommended order: `platform-basics` → `training-manual-lifecycle` → `training-hpo` → `evaluation` → `model-export-extras`. 5 model artifacts + 5 datasets + 5 projects from this session are reusable for `evaluation.md` + `model-export-extras.md` |
| MT.7 | Open PR `feature/training-eval-smoke` → `dev` | ⏳ | After parks confirms all 5 runbook walks green. Expected clean fast-forward (10 commits, no conflicts) |
