...
export interface Project {
  ...
  description: string | null
  task_type: TaskType
  /** Opaque ID from an external system (e.g. a companion frontend's project row) this
...
export interface ExportProgressMsg extends WSMessageBase {
  type: 'export_progress'
  stage: 'downloading' | 'merging' | 'converting' | 'quantizing' | 'uploading' | 'registering'
  detail?: string | null
}

export interface EvaluationProgressMsg extends WSMessageBase {
  type: 'evaluation_progress'
  phase: 'predicting' | 'scoring' | 'judging'
  rows_done: number
  rows_total: number
}
